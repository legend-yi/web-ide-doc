# EOSIO Web IDE教程：只需几分种便能在EOSIO上开发

## 前言
开始使用一项新技术总是会带来挑战。通常需要学习新的语言、开发模式还要设置开发环境所要求的硬件条件。特别是目前在区块链开发具有某些资源要求，必须满足这些资源要求才能运行完整节点、重放链上历史或测试智能合约。

以EOS为例，对于开发者来说，首先需要安装和配置EOSIO，之后，必须安装和配置EOSJS才能开发Web应用程序。因为这些过程需要许多步骤，其中包括许多组件。还有计算能力的问题。要运行完整的节点，需要具有至少16 GB RAM的计算机，通常需要32 GB或更大的RAM、更大的磁盘空间以及运行速度更快的CPU。

为此EOSIO团队在2.0版本推出了新的工具和库，使EOSIO开发更快、更容易，并且更容易被世界各地的开发人员访问。EOS Web IDE简化了区块链开发人员的障碍。

## IDE是什么？

主要是用于提供程序开发环境的应用程序，一般包括代码编辑器、编译器、调试器和图形用户界面等工具。

## Web IDE是什么？

Web IDE是将IDE里面除用户交互之外的功能全部移到了后台，开发人员在浏览器上可以像在本机一样调试、编译、运行程序，并且不会受系统环境和机器性能的限制。目前的web IDE主要有**AWS Cloud9、Cloud Studio、Eclipse Che、Gitpod**等，这些web IDE各有千秋，其中Gitpod是目前web IDE中对Github上的项目支持最好的，也是eosio主推的一款产品。并且Gitpod对于非商用的开发者，每月会有100小时的免费使用时间，本文的讲解，也会以Gitpod为例（下文中提到的web IDE，在没有特指的情况下，全部为Gitpod）

![web-ide-arch](https://github.com/Gatsbyter/web-ide-doc/raw/master/images/web-ide-arch.png)
...
点击Gitpod按钮，就可以直接进到一个节点已经启动好的环境中，并且这个环境提供一些编写智能合约的模板（实际的开发环境中，建议大家先fork这个项目，然后再启动Gitpod）。

从图中可以看见，eos节点已经启动并且正在出块，同时整个编写智能合约的IDE环境已经搭建好，现在就可以使用web IDE直接开始开发智能合约。

### 3. 开发调试合约

智能合约的具体开发、调试以及部署说明可以参考[eosio白皮书]，本文仅分析该项目模板合约的部署和调用
![web-ide](https://github.com/Gatsbyter/web-ide-doc/raw/master/images/web-ide.png)
...
[[eosio::action]] void post(uint64_t id, uint64_t reply_to, eosio::name user, const std::string& content) {
        ...
        table.emplace(get_self(), [&](auto& message) {
            message.id       = id;[com](aaa%E3%80%82)
...
* 理清post方法的逻辑之后，就可以开始编译文件，首先点击Gitpod上方导航栏的Terminal -> New Terminal(如下图)

![terminal](https://github.com/Gatsbyter/web-ide-doc/raw/master/images/terminal.png)

可以看见Gitpod下方出现了一个新的命令行窗口

![command](https://github.com/Gatsbyter/web-ide-doc/raw/master/images/command.png)
...
目前我们可以看到EOSIO正在添加许多新功能，将来可能会支持Swift和Java应用程序开发的支持，用于保护知识产权的基于Theia的本地版本，附加的UI功能以及允许开发人员与数据进行交互的桌面浏览器。同时，EOSIO正在考虑集成基于Web的区块浏览器，DEMUX和历史记录工具。
