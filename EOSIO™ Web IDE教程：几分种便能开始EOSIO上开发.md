...
开始使用一项新技术总是会带来挑战。通常需要学习新的语言、开发模式还要设置开发环境所要求的硬件先决条件。特别是目前在区块链开发具有某些资源要求，必须满足这些资源要求才能运行完整节点、重放链上历史或测试智能合约。
以EOS为例，基于EOSIO的开发需要多步骤的设置，以及一台功能强大的计算机来运行完整的区块链节点。对于开发者来说，首先需要安装和配置EOSIO，之后，必须安装和配置EOSJS才能开发Web应用程序。因为这些过程需要许多步骤，其中包括许多组件。还有计算能力的问题。要运行完整的节点，需要具有至少16 GB RAM的计算机，通常需要32 GB或更大的RAM、更大的磁盘空间以及运行速度更快的CPU。
...
目前EOSIO正在研究许多功能，将来EOSIO Quickstart Web IDE可能会支持Swift和Java应用程序开发的支持，用于保护知识产权的基于Theia的本地版本，附加的UI功能以及允许开发人员与数据进行交互的桌面浏览器。同时，EOSIO正在考虑集成基于Web的区块浏览器，DEMUX和历史记录工具。
